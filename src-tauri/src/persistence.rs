...
pub fn load_state(connection: &Connection) -> Result<FocusFlowPersistedState, rusqlite::Error> {
    let (mood, xp, pet_name): (String, i64, String) = connection.query_row(
        "SELECT mood, xp, pet_name FROM app_state WHERE id = 1",
        [],
...
pub fn save_state(connection: &mut Connection, state: &FocusFlowPersistedState) -> Result<(), rusqlite::Error> {
    ...
    transaction.execute(
        "
        INSERT INTO app_state (id, mood, xp, pet_name)
        VALUES (1, ?1, ?2, ?3)
        ON CONFLICT(id) DO UPDATE SET mood = excluded.mood, xp = excluded.xp, pet_name = excluded.pet_name
        ",
        params![state.mood, state.xp, pet_name],
    )?;
