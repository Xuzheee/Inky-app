...
- `claude.rs` owns optional AI parsing configuration and network calls. Personal DeepSeek keys and invite proxy config are stored in `ai-config.json` under the app local data directory; the frontend should only receive config status, never secret values.
...
- When changing AI parsing result shape, update Rust normalization in `claude.rs` and frontend normalization in `aiParser.ts` together.
